...
with open('config_push.txt','r') as file1:
...
def exe_commands(hostname,command):
    try:
        ...
        with open(current_conf_file,'w') as conf_file:           # open the file to write 
            for cmd in command:             # for executing comands

                    int_config.send(f" {cmd} ")         # remove the  \n from the send of the command bcoz it is already there in the input file
                    time.sleep(1)
                    output=int_config.recv(65535).decode('utf-8')
                    conf_file.write(output)                              # store the precheck output in the file
                    print(output)
                    
            int_config.send(" show run \n")                # for sh run commmands
            time.sleep(1)                      
            output=int_config.recv(65535).decode('utf-8')
            conf_file.write(output)                              # store the run command output in the file
            print(output)
